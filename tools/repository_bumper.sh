...
# fallback in notifications/build.gradle to match, then (depending on flags)
# reinitializes CHANGELOG.md and pins workflow references to the right branch/tag.
#
...
# ====
# Sync the hardcoded version fallback in notifications/build.gradle to the release
# version. build.gradle resolves the build version from the `version` system
# property and falls back to a hardcoded default when it is not passed. Keeping
# that default equal to VERSION.json stops it drifting behind the real version and
# requesting a common-utils artifact (${wazuh_version}.${revision}) that was never
# published (e.g. a bare `./gradlew` with no -Dversion). The Gradle build lives in
