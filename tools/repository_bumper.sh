...
# =========================
# Updates VERSION.json for a new version release, checks that
# notifications/build.gradle still resolves its version from it, then (depending on
# flags) reinitializes CHANGELOG.md and pins workflow references to the right
# branch/tag.
#
...
# ====
# Assert notifications/build.gradle does not reintroduce a hardcoded version
# fallback. build.gradle resolves the build version from the `version` system
# property and falls back to reading VERSION.json when it is not passed (e.g. a bare
# `./gradlew` with no -Dversion), so VERSION.json is the single source of truth and
# there is nothing for this script to sync. A hardcoded literal coming back means
# the fallback can drift behind VERSION.json again and request a common-utils
# artifact (${wazuh_version}.${revision}) that was never published, so fail rather
# than warn. The Gradle build lives in the notifications/ subdirectory.
# ====
function check_build_gradle_version() {
    local file="notifications/build.gradle"
    ...
    log "Verified $file resolves the version from VERSION.json."
}
